...
class transfers_reservations(Model):
    _name = "transfers.reservations"
    _description = "Transfers Reservations"
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        if view_type == 'tree':
            self.env.cr.execute(
                'DELETE FROM transfers_reservations WHERE id IN ( SELECT id FROM transfers_reservations)')
            sol = self.env['sale.order.line']
            for line in sol.search([]):
                if line.category.lower() == 'transfer':
                    base_dict = {
                    ...
                    for package_line in line.sale_order_line_package_line_id:
                        if package_line.category_id.name.lower() == 'transfer':
                            base_dict = {
                                'sale_order_line_id': line.id,
                            ...
                            self.create(base_dict)
        return super(transfers_reservations, self).fields_view_get(view_id=view_id, view_type=view_type,
                                                                   toolbar=toolbar,
                                                                   submenu=submenu)
