...
class Website(openerp.addons.web.controllers.main.Home):
    ...
    def pagenew(self, path, noredirect=False, add_menu=None):
        ...
        if add_menu:
            model, id = request.registry["ir.model.data"].get_object_reference(request.cr, request.uid, 'website',
                                                                               'main_menu')
    ...
    def reset_template(self, templates, redirect='/'):
        ...
        if modules_to_update:
            ...
            module_ids = module_obj.search(request.cr, request.uid, [('name', 'in', modules_to_update)],
                                           context=request.context)
            if module_ids:
    ...
    def get_website_translations(self, lang):
        module_obj = request.registry['ir.module.module']
        module_ids = module_obj.search(request.cr, request.uid,
                                       [('name', 'ilike', 'website'), ('state', '=', 'installed')],
                                       context=request.context)
        modules = [x['name'] for x in
                   module_obj.read(request.cr, request.uid, module_ids, ['name'], context=request.context)]
    ...
    def actions_server(self, path_or_xml_id_or_id, **post):
        ...
        if isinstance(path_or_xml_id_or_id, basestring) and '.' in path_or_xml_id_or_id:
            action_id = request.registry['ir.model.data'].xmlid_to_res_id(request.cr, request.uid, path_or_xml_id_or_id,
                                                                          raise_if_not_found=False)
        if not action_id:
            action_ids = ServerActions.search(cr, uid, [('website_path', '=', path_or_xml_id_or_id),
